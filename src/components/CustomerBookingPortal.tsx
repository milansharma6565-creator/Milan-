...
import { Phone, CheckCircle2, Navigation, MapPin, AlertCircle, Calendar, Truck, Lock, User as UserIcon, Plus, X, Receipt, QrCode, FileText, MessageCircle, Send, Bell, Droplets, FlaskConical as Flask, Package, Smartphone, Mail } from 'lucide-react';
...
export function CustomerBookingPortal() {
  ...
  const [loginStep, setLoginStep] = useState<'PHONE_INPUT' | 'OTP_VERIFY' | 'REGISTER_NEW'>('PHONE_INPUT');
  ...
  // Interactive 3D tilt state for cinematic premium experience
  ...
  if (!isLogged) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 flex items-center justify-center p-4 relative overflow-hidden select-none">
        {/* Dynamic Glowing Ambient Orbs */}
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full bg-blue-500/10 blur-[120px] pointer-events-none animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-indigo-500/10 blur-[120px] pointer-events-none animate-pulse" style={{ animationDelay: '2s' }} />
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {[...Array(12)].map((_, idx) => (
            <motion.div
              ...
              className="absolute w-12 h-12 rounded-full bg-gradient-to-tr from-cyan-400/20 to-blue-500/10 backdrop-blur-xs ring-1 ring-white/10"
              style={{
        ...
        >
          <motion.div 
            ...
            style={{
              ...
              boxShadow: '0 30px 100px -15px rgba(0, 0, 0, 0.7), 0 0 50px -10px rgba(99, 102, 241, 0.15)',
              border: '1px solid rgba(255, 255, 255, 0.08)'
            }}
            initial={{ opacity: 0, y: 50, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
            className="bg-slate-900/40 backdrop-blur-2xl p-8 sm:p-10 rounded-[3rem] w-full text-center relative overflow-hidden"
          >
            {/* Holographic Glowing Top Bar */}
            <div className="absolute top-0 inset-x-0 h-[3px] bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-600 shadow-[0_2px_15px_rgba(14,165,233,0.5)] z-20" />
            ...
            <div 
              ...
              className="bg-slate-950 w-24 h-24 rounded-[2.2rem] flex items-center justify-center mx-auto mb-6 shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 relative z-10 transition-transform hover:scale-105"
            >
               <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/10 to-indigo-500/10 rounded-[2.2rem] opacity-50 pulse" />
               <Logo size={48} color="white" />
            </div>
            
            <div style={{ transform: 'translateZ(30px)' }} className="relative z-10">
              <h2 className="text-3xl font-black text-white mb-1.5 tracking-tight font-sans">
                Tanker<span className="relative text-cyan-400">Wala<span className="absolute top-full left-0 text-[10px] text-slate-400 font-bold tracking-widest uppercase mt-0.5 opacity-60">Powered by Rajhans</span></span>
              </h2>
              
              <p className="text-[10px] font-black text-cyan-400/70 uppercase tracking-[0.25em] mt-7 mb-8 font-sans">
                {loginStep === 'PHONE_INPUT' ? 'Enter Mobile Number' : 
                 loginStep === 'OTP_VERIFY' ? 'Verify OTP Code' : 
                 'Complete Registration'}
              </p>
            </div>
            
            <div style={{ transform: 'translateZ(20px)' }} className="space-y-5 mb-8 text-left relative z-10">
              {loginStep === 'PHONE_INPUT' && (
                <div className="space-y-4">
                  <div className="text-center py-3 px-4 bg-slate-950/25 border border-white/5 rounded-2xl text-slate-300 text-xs font-medium leading-relaxed font-sans mb-3">
                    Welcome to <span className="font-extrabold text-white">Rajhans TankerWala</span>. Book premium water tankers, cans, and bottles in 1-Click with live-tracking using your mobile.
                  </div>
                  
                  <div className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <span className="text-cyan-400 font-black text-sm font-sans">+91</span>
                    </div>
                    <input
                      type="tel"
                      value={mobileNumber}
                      onChange={(e) => setMobileNumber(e.target.value.replace(/\D/g, '').slice(0, 10))}
                      className="w-full pl-14 pr-4 py-4 bg-slate-950/40 border-2 border-white/5 rounded-2xl text-lg font-extrabold text-white placeholder-slate-600 focus:border-cyan-500 focus:bg-slate-950/60 shadow-inner group-hover:border-white/10 transition-all outline-none font-sans"
                ...
                <div className="space-y-4">
                  {showOtpHint && (
                    <motion.div 
                      ...
                      className="p-4 bg-slate-950/30 border border-cyan-500/20 rounded-2xl mb-4 text-xs text-cyan-200 leading-relaxed font-sans"
                    >
                      <span className="font-extrabold text-cyan-400 block mb-1.5 flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-cyan-400 animate-ping inline-block" />
                        💬 Rajhans OTP Simulator:
                      </span>
                      Your 6-digit confirmation code is <span className="font-mono bg-cyan-950/80 text-cyan-300 px-2.5 py-1 rounded-lg border border-cyan-500/20 font-black text-sm">{generatedOtp}</span>. Enter it below to proceed.
                    </motion.div>
                  )}
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-4 bg-slate-950/40 border-2 border-white/5 rounded-2xl text-xl font-black tracking-[0.25em] text-white text-center focus:border-cyan-500 focus:bg-slate-950/60 transition-all outline-none font-sans"
                      placeholder="______"
                      required
                    />
                  </div>
              ...
              {loginStep === 'REGISTER_NEW' && (
                <div className="space-y-4">
                  <p className="text-xs text-slate-300 font-bold leading-normal mb-2 bg-slate-950/20 p-3 rounded-2xl border border-white/5 font-sans">
                    👋 <span className="text-cyan-400 font-black">Account is not registered yet.</span> Welcome to Rajhans! Please complete your registration details to continue.
                  </p>

                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <UserIcon size={18} className="text-cyan-500/80" />
                    </div>
                    <input
                      type="text"
                      value={newName}
                      onChange={(e) => setNewName(e.target.value)}
                      className="w-full pl-12 pr-4 py-4 bg-slate-950/40 border-2 border-white/5 rounded-2xl text-base font-bold text-white placeholder-slate-500 focus:border-cyan-500 focus:bg-slate-950/60 transition-all outline-none font-sans"
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <Mail size={18} className="text-cyan-500/80" />
                    </div>
                    <input
                      type="email"
                      value={newEmail}
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <MapPin size={18} className="text-cyan-500/80" />
                    </div>
                    <input
                      type="text"
                      value={newAddress}
              ...
              {error && (
                <motion.div 
                  ...
                  className="text-red-400 text-xs font-bold bg-red-950/30 p-4 rounded-2xl flex items-center gap-2.5 border border-red-500/20 font-sans"
                >
            ...
            <div style={{ transform: 'translateZ(25px)' }} className="relative z-10">
              {loginStep === 'PHONE_INPUT' && (
                <button 
                  ...
                  className={`w-full h-15 rounded-2xl font-black flex items-center justify-center gap-2 transition-all font-sans text-sm ${
                    loading || mobileNumber.length < 10
                      ? 'bg-slate-900/80 text-slate-600 border border-white/5 cursor-not-allowed' 
                      : 'bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-600 text-white shadow-xl shadow-cyan-950/30 hover:brightness-110 active:scale-95 cursor-pointer border border-cyan-400/20'
                  }`}
                >
                  {loading ? (
              ...
              {loginStep === 'OTP_VERIFY' && (
                <div className="space-y-3">
                  ...
                  <button 
                    ...
                    className="w-full text-xs font-black text-slate-400 hover:text-slate-200 transition tracking-wider uppercase font-sans py-2"
                  >
                    Go Back / Edit Number
                  </button>
                </div>
            ...
            <div style={{ transform: 'translateZ(15px)' }} className="mt-8 flex flex-col gap-1.5 items-center justify-center p-4 bg-slate-950/30 rounded-2xl border border-white/5 font-sans relative z-10">
               ...
               <p className="text-[10px] text-slate-400 font-medium block leading-normal opacity-80">100% passwordless authentication protects your account security.</p>
            </div>
          </motion.div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 text-white relative overflow-x-hidden selection:bg-cyan-500/30">
      <WishesOverlay />
        ...
        {activeAlarmBill && (
          ...
          >
            <div className="bg-slate-900/90 backdrop-blur-3xl rounded-[3rem] p-10 shadow-2xl max-w-sm w-full relative overflow-hidden border border-white/10 text-white">
              ...
              <h2 className="text-3xl font-display font-black text-white mb-2">Driver Reached!</h2>
              <p className="text-slate-300 font-medium mb-8">
                Your water tanker <span className="font-bold text-cyan-400">{activeAlarmBill.billNumber}</span> has arrived at your location.
              </p>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="font-display font-black text-base leading-none text-white pb-1 flex items-center gap-1.5">
                Tanker<span className="text-cyan-400 relative">Wala<span className="absolute top-[90%] left-0 text-[8px] text-slate-400 font-bold whitespace-nowrap tracking-wider uppercase">Powered by Rajhans</span></span>
              </h1>
              <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mt-1.5">{customer?.name}</p>
            </div>
          </div>
      ...
      <main className="max-w-md mx-auto p-4 pb-24 space-y-6 relative z-10" style={{ perspective: '1200px' }}>
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-b from-cyan-500/[0.04] to-transparent pointer-events-none" />
           ...
           <h3 className="text-2xl font-display font-black text-white tracking-tight text-center">Premium Water Flow</h3>
           <p className="text-[10px] font-extrabold text-cyan-400 uppercase tracking-[0.25em] mt-1.5">Smart Distribution Network</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {activeSlide === 'TANKER' && (
            <motion.div 
              ...
              initial={{ opacity: 0, rotateY: 35, translateZ: -80, scale: 0.95 }}
              ...
              <div className="bg-gradient-to-br from-cyan-950/40 via-blue-900/30 to-slate-900/40 backdrop-blur-2xl rounded-[2.5rem] p-6 text-white border border-cyan-500/10 shadow-[0_20px_50px_rgba(6,182,212,0.12)]">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-extrabold text-sm text-cyan-100 uppercase tracking-widest">{format(new Date(), 'MMMM yyyy')}</span>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="text-3xl font-black text-white">{formatCurrency(analytics.TANKER.spent)}</div>
                    <div className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest">{analytics.TANKER.trips} Trips Delivered</div>
                  </div>
                  <button 
              ...
              <div className="bg-gradient-to-br from-orange-950/40 via-amber-900/30 to-slate-900/40 backdrop-blur-2xl rounded-[2.5rem] p-6 text-white border border-orange-500/10 shadow-[0_20px_50px_rgba(249,115,22,0.12)]">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-extrabold text-sm text-orange-100 uppercase tracking-widest">20L RO Cans</span>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="text-3xl font-black text-white">{formatCurrency(analytics.CAN.spent)}</div>
                    <div className="text-[10px] font-bold text-orange-300 uppercase tracking-widest">{analytics.CAN.trips} Deliveries</div>
                  </div>
              ...
              <div className="bg-gradient-to-br from-emerald-950/40 via-green-900/30 to-slate-900/40 backdrop-blur-2xl rounded-[2.5rem] p-6 text-white border border-green-500/10 shadow-[0_20px_50px_rgba(34,197,94,0.12)]">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-extrabold text-sm text-emerald-100 uppercase tracking-widest">Packaged Items</span>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-[10px] font-bold text-emerald-300 uppercase tracking-widest">{analytics.BOTTLE.trips} bundles ordered</div>
                  </div>
                  <div className="grid grid-cols-2 gap-3 mt-4">
                    <button 
          ...
          <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2.5rem] p-6 border border-white/5 space-y-6 shadow-2xl">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Choose a premium service</p>
              </div>
            </div>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x">
              {allowedServices.tanker && (
                ...
                >
                  ...
                  <div className="text-center">
                    <span className="text-[11px] font-black uppercase tracking-wider text-white block">Tanker</span>
              ...
              {allowedServices.can && (
                ...
                >
                  ...
                  <div className="text-center">
                    <span className="text-[11px] font-black uppercase tracking-wider text-white block">20L Can</span>
              ...
              {allowedServices.bottle && (
                ...
                >
                  ...
                  <div className="text-center">
                    <span className="text-[11px] font-black uppercase tracking-wider text-white block">Packaged</span>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              <div className="bg-cyan-500/5 p-4 rounded-[1.5rem] border border-cyan-500/10 shadow-sm relative overflow-hidden">
                 <div className="text-[10px] font-black text-cyan-400 uppercase tracking-widest mb-1">Fast Delivery</div>
                 <div className="text-sm font-bold text-white">Under 90 Mins</div>
              </div>
              <div className="bg-emerald-500/5 p-4 rounded-[1.5rem] border border-emerald-500/10 shadow-sm relative overflow-hidden">
                 <div className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">Purity Checked</div>
          ...
          <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2.5rem] p-6 border border-white/5 shadow-2xl space-y-6">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Available sub-sections</p>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-black text-white leading-tight">Trip Tanker</div>
                    <p className="text-[10px] text-slate-450 font-medium leading-tight mt-1">One-time bulk delivery for house filling & construction.</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-black text-white leading-tight">Day Tanker (Standby)</div>
                    <p className="text-[10px] text-slate-450 font-medium leading-tight mt-1">Perfect for marriage functions & events. Tanker stays with you.</p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="font-black text-white leading-tight">Monthly Booking</div>
                    <p className="text-[10px] text-slate-450 font-medium leading-tight mt-1">Subscription-based regular water supply for commercial use.</p>
                  </div>
                </button>
              </div>
          </div>
          ...
          <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2.5rem] p-6 border border-white/5 shadow-2xl">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">RO Chilled Selection</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[9px] text-blue-100 font-medium leading-tight mt-1.5 px-1 capitalize">Standard Chilled RO Water at your door.</p>
                </div>
              </button>
              ...
              >
                ...
                <div className="relative z-10">
                   ...
                   <p className="text-[9px] font-bold text-orange-400 uppercase mt-1.5 tracking-wider">Help poor people on road.</p>
                </div>
              </button>
            </div>
            ...
            <div className="bg-cyan-500/5 rounded-[2rem] p-5 text-white mb-6 border border-cyan-500/10 relative overflow-hidden shadow-inner">
              <div className="flex items-center justify-between mb-4">
                 <span className="text-[10px] font-black uppercase tracking-widest text-cyan-400">Total Spent (Cans)</span>
              ...
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{analytics.CAN.trips} Total Delivered</div>
            </div>

              <div className="mb-8">
                <h4 className="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2">
                  <Calendar size={14} className="text-cyan-400" /> 30-Day Delivery Calendar
                </h4>
                ...
                <div className="mt-8 bg-slate-950/40 border border-white/5 rounded-3xl p-6 shadow-inner">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h4 className="text-sm font-black text-white leading-none">Next Day Quantity</h4>
                      <p className="text-[10px] font-bold text-slate-450 uppercase tracking-widest mt-1.5">For Monthly Users</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 bg-slate-900/40 p-2 rounded-2xl border border-white/5">
                    <div className="flex-1 flex flex-col pl-2">
                      <span className="text-[9px] font-black text-slate-450 uppercase tracking-widest leading-none">Requested Cans</span>
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-center gap-2">
                     ...
                     <p className="text-[9px] font-bold text-cyan-400 uppercase tracking-widest">Notification will be sent tonight at 9 PM</p>
                  </div>
                </div>
              </div>
            ...
            <div 
              ...
              className="mt-8 bg-orange-500/5 rounded-[2rem] p-6 border border-orange-500/10 relative overflow-hidden cursor-pointer hover:bg-orange-550/10 transition-colors shadow-2xl"
            >
                 ...
                 <AnimatePresence>
                   {showDonationQR && (
                     ...
                     >
                       ...
                       <p className="text-orange-400 font-extrabold text-xs uppercase tracking-widest text-center mt-2">Scan to Donate Directly</p>
                     </motion.div>
                   )}
                 </AnimatePresence>
              ...
              <div className="mt-6 bg-slate-950/40 rounded-3xl p-5 border border-white/5 text-white text-left shadow-2xl shadow-inner">
                <div className="flex items-center gap-2 mb-4">
                  <span className="w-2.5 h-2.5 bg-yellow-500 rounded-full animate-pulse" />
                   <h4 className="text-sm font-black text-white uppercase tracking-tight">Pending Bookings ({bookingRequests.length})</h4>
                </div>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-3">Awaiting Franchise Confirmation</p>
                <div className="space-y-3">
                  {bookingRequests.map((req) => (
                    <div key={req.id} className="bg-slate-900/40 p-4 rounded-2xl border border-white/5 flex flex-col justify-between md:flex-row md:items-center gap-3 shadow-md hover:border-yellow-500/30 transition-all">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {req.quantity && (
                            <span className="text-[10px] text-cyan-400 font-bold">Qty: {req.quantity}</span>
                          )}
                          {req.tankerSize && (
                            <span className="text-[10px] text-purple-400 font-bold">({req.tankerSize})</span>
                          )}
                        </div>
                        ...
                        {req.remark && (
                          <p className="text-[10px] text-slate-300 italic mt-1 font-medium">"{req.remark}"</p>
                        )}
                      </div>
                      <button
          ...
          <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2rem] p-6 border border-white/5 shadow-2xl">
             <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Bundles & Cases</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-emerald-950/40 via-green-900/30 to-slate-900/40 rounded-[2rem] p-5 text-white mb-6 border border-green-500/10 shadow-xl shadow-inner">
              <div className="flex items-center justify-between mb-4">
                 <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">Total Spent (Bottles)</span>
              ...
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{analytics.BOTTLE.trips} Total Bundles</div>
            </div>

            <div className="mb-8">
              <h4 className="text-xs font-black text-white uppercase tracking-widest mb-4 flex items-center gap-2">
                <Smartphone size={14} className="text-emerald-400" /> Case/Bundle Delivery Log
              </h4>
            ...
            >
               ...
               <div>
                  <h3 className="text-xl font-black text-white mb-1">Bottle Bundles</h3>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Available in 500ml, 1L, 2L</p>
               </div>
           ...
           <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2rem] p-6 border border-white/5 space-y-6 shadow-2xl">
              <div className="flex items-center gap-3 border-b border-white/5 pb-4">
                <button onClick={() => setSelectedCategory(null)} className="p-2 bg-white/5 text-slate-400 hover:text-white rounded-xl transition-colors cursor-pointer">
                  <X size={16} />
                </button>
                <div>
                  <h2 className="font-bold text-white">Humanity First</h2>
                  <p className="text-xs text-slate-400 font-medium">Donate water for the needy</p>
                </div>
              </div>
              ...
              <div className="text-center py-4">
                ...
                <h3 className="text-base font-black text-orange-400 mb-2 italic">"Pyaase ko paani mil jaye, to dua baras jaayegi"</h3>
                <p className="text-xs text-slate-300 font-medium px-4">Choose an amount to donate. We will place RO water cans at our roadside kiosks for free public use.</p>
              </div>
              ...
              <div className="bg-slate-950/60 p-6 rounded-[2rem] border border-white/5 text-white">
                 ...
                 <p className="text-[9px] text-slate-400 text-center mt-3 font-bold uppercase tracking-widest">Payment secured via UPI Dashboard</p>
              </div>
           </div>
          ...
          <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2rem] p-6 border border-white/5 space-y-5 shadow-2xl">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Select location & details</p>
              </div>
            </div>

            <div>
               <label className="block text-xs font-bold text-slate-300 uppercase tracking-wide mb-2 flex items-center justify-between">
                 <span>Delivery Location <span className="text-cyan-400">*</span></span>
                 <span className="text-[10px] text-cyan-400 font-black tracking-wider uppercase">Drag pin to exact point</span>
               </label>
               ...
               {location && (
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="bg-cyan-500/10 text-cyan-400 p-3 rounded-xl text-xs font-bold flex items-center justify-between border border-cyan-500/25">
                      ...
                      <span className="text-sm font-black">{distanceKm.toFixed(1)} KM</span>
                    </div>
                  </div>
               )}
            </div>
            ...
            {location && (
              ...
              >
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-300 uppercase tracking-wide mb-3 flex items-center gap-2">
                        <AlertCircle size={14} className="text-orange-400 animate-pulse" /> Delivery Plan
                      </label>
                      <button 
                      ...
                      >
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-md ${isFastDelivery ? 'bg-gradient-to-r from-orange-500 to-amber-500 text-white' : 'bg-slate-900 border border-white/5 text-slate-400'}`}>
                          <Truck size={24} className={isFastDelivery ? "animate-pulse" : ""} />
                        </div>
                        <div className="flex-1">
                          <div className="text-sm font-black text-white">Fastest Emergency Delivery</div>
                          <div className={`text-[10px] font-bold uppercase tracking-wider ${isFastDelivery ? 'text-orange-450' : 'text-slate-400'}`}>Requires Pipe & adds +₹100 charge</div>
                        </div>
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${isFastDelivery ? 'border-orange-500 bg-orange-500 text-white' : 'border-slate-700'}`}>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-300 uppercase tracking-wide mb-2">Delivery Floor</label>
                    <select 
                      value={floors}
                ...
                {selectedCategory === 'TANKER' && (
                   <div>
                    <label className="block text-xs font-bold text-slate-300 uppercase tracking-wide mb-2 flex items-center justify-between">
                      ...
                      <span className="text-cyan-400 bg-cyan-500/10 border border-cyan-500/25 px-2 py-0.5 rounded-full">{pipeLength} ft</span>
                    </label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-300 uppercase tracking-wide mb-2">Special Remarks (Optional)</label>
                  <textarea
                    value={remarks}
                ...
                {activeFranchise?.loyaltyProgramEnabled && customer && (customer.loyaltyCoins || 0) > 0 && (
                  <div className="bg-gradient-to-r from-amber-500/5 to-orange-500/5 border border-amber-500/20 rounded-2xl p-4 flex items-center justify-between shadow-inner">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-black text-amber-450 leading-none">Redeem Loyalty Coins</h4>
                        <p className="text-[10px] font-bold text-amber-300/80 uppercase tracking-wider mt-1.5">₹1 = 1 Coin (Balance: {customer.loyaltyCoins || 0})</p>
                      </div>
                    </div>
                    <button
                      type="button"
          ...
          <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2rem] p-6 border border-white/5 space-y-6 shadow-2xl">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Select size and quantity</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {(['500ml', '1L', '2L'] as const).map(size => (
                ...
                >
                  <div className={`p-2 rounded-lg ${bottleSize === size ? 'bg-cyan-500 text-white shadow-sm' : 'bg-slate-900 border border-white/5 text-slate-400'}`}>
                    <Droplets size={20} />
                  </div>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-350 uppercase tracking-wide mb-2">Number of Cases / Bundles</label>
              <div className="flex items-center gap-4">
                <button 
                  onClick={() => setQuantity(q => q > 1 ? q - 1 : 1)}
                  className="w-12 h-12 bg-slate-850 hover:bg-slate-750 text-white rounded-xl flex items-center justify-center font-bold text-lg active:scale-95 transition-all cursor-pointer border border-white/5"
                ...
                {location && (
                   <div className="space-y-2">
                     <div className="bg-green-500/10 text-green-450 p-2 rounded-xl text-xs font-medium flex items-center gap-2 border border-green-500/20 justify-center">
                       ...
                       <span className="line-clamp-1">{location.lat.toFixed(6)}, {location.lng.toFixed(6)}</span>
                     </div>
                   </div>
                )}
            </div>
            ...
            {activeFranchise?.loyaltyProgramEnabled && customer && (customer.loyaltyCoins || 0) > 0 && (
              <div className="bg-gradient-to-r from-amber-500/5 to-orange-500/5 border border-amber-500/20 rounded-2xl p-4 flex items-center justify-between shadow-inner">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-white leading-none">Redeem Loyalty Coins</h4>
                    <p className="text-[10px] font-bold text-amber-300/80 uppercase tracking-wider mt-1.5">You have: {customer.loyaltyCoins || 0} Coins</p>
                  </div>
                </div>
                <button
                  type="button"
          ...
          <div className="bg-slate-900/30 backdrop-blur-2xl rounded-[2rem] p-6 border border-white/5 space-y-6 shadow-2xl">
            <div className="flex items-center gap-3 border-b border-white/5 pb-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">RO Chilled Water</p>
              </div>
            </div>
            ...
            <div className="bg-cyan-500/5 p-6 rounded-[2rem] flex flex-col items-center text-center border border-cyan-500/10 relative overflow-hidden shadow-inner">
               ...
               <h3 className="font-black text-base text-white relative z-10">RO 20 Ltr Can</h3>
               <p className="text-xs text-slate-400 font-bold mt-2 relative z-10">Base ₹30 + ₹10/KM Delivery</p>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-305 uppercase tracking-wide mb-2">Quantity of Cans</label>
              <div className="flex items-center gap-4">
                <button 
                  onClick={() => setQuantity(q => q > 1 ? q - 1 : 1)}
                  className="w-12 h-12 bg-slate-850 hover:bg-slate-750 text-white rounded-xl flex items-center justify-center font-bold text-lg active:scale-95 transition-all cursor-pointer border border-white/5"
        ...
        {bills.length > 0 && (
          <div className="mt-8">
            <h3 className="font-display font-medium text-lg mb-4 text-slate-300">Recent Trips</h3>
            <div className="space-y-4">
              {bills.slice(0, 5).map(bill => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-sm font-bold text-white border-b border-white/10 pb-0.5">{bill.billNumber}</span>
                        <span className={`text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider ${
                          bill.status === 'Delivered' || bill.status === 'Printed' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                          (bill.status === 'Pending' || bill.status === 'Filling' || bill.status === 'Assigned' || bill.status === 'On the way') ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' : 
                          bill.status === 'Reached' ? 'bg-cyan-500 text-slate-950 animate-pulse' : 'bg-red-500/10 text-red-400 border border-red-500/20'
                        }`}>
                          {bill.status}
                        </span>
                      </div>
                    ...
                    <div className="text-right">
                      <div className="font-black text-white border-b border-white/5 pb-0.5 mb-1">{formatCurrency(bill.grandTotal)}</div>
                      <div className="text-[10px] text-slate-400 font-medium">{bill.paymentMode}</div>
                    </div>
                  </div>
                          ...
                          {[
                            ...
                            return (
                              <div key={item.label} className="flex flex-col items-center relative z-10">
                                ...
                                <span className={`text-[8px] font-black mt-2 uppercase tracking-widest ${active ? 'text-cyan-400' : 'text-slate-500'}`}>
                                  {item.label}
                                </span>
                              </div>
                            );
                      ...
                      {bill.driverName && (
                        <div className="flex items-center justify-between bg-blue-500/5 text-blue-300 p-3 rounded-xl mb-2 border border-blue-500/10">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <div className="text-[10px] font-bold text-blue-450 uppercase tracking-wider">Driver Assigned</div>
                              <div className="text-xs font-black text-white">{bill.driverName}</div>
                            </div>
                          </div>
                          {bill.driverMobile && (
                            <a href={`tel:${bill.driverMobile}`} onClick={(e) => e.stopPropagation()} className="flex items-center justify-center gap-1.5 bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-blue-700 transition-colors shadow-md shadow-blue-500/20">
                              <Phone size={12} /> Call
                            </a>
                          )}
                        </div>
                      ...
                      {bill.driverId && driverLocations[bill.driverId] && (
                        <div className="bg-slate-950/40 rounded-2xl p-3 mb-2 border border-white/5">
                           <div className="flex items-center justify-between mb-2">
                             ...
                             <span className="text-[10px] font-bold text-cyan-400">Speed: {Math.round(driverLocations[bill.driverId].speed || 0)} km/h</span>
                           </div>
                           <div className="h-32 rounded-xl overflow-hidden relative border border-white/5">
                             <div className="absolute inset-0 flex items-center justify-center bg-cyan-950/20">
                                <div className="text-center p-4">
                                   <MapPin className="text-cyan-400 mx-auto mb-2 animate-bounce" size={24} />
                                   <p className="text-[10px] font-bold text-slate-400">Driver is nearby</p>
                                   <button 
                                     onClick={(e) => {
                                       e.stopPropagation();
                      ...
                      >
                        <div className="grid grid-cols-4 gap-2 mt-4 pt-4 border-t border-white/5">
                           {bill.status === 'Delivered' && (
                             <button onClick={(e) => { e.stopPropagation(); setModalView({ type: 'BILL', bill }); }} className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl border border-white/5 bg-slate-950/30 hover:bg-slate-900/40 hover:border-white/10 transition-all text-slate-300 cursor-pointer">
                               <Receipt size={18} className="text-cyan-400 opacity-80" />
                               <span className="text-[10px] font-bold">View Bill</span>
                             </button>
                           )}
                           <button onClick={(e) => { e.stopPropagation(); setModalView({ type: 'QR', bill }); }} className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl border border-white/5 bg-slate-950/30 hover:bg-slate-900/40 hover:border-white/10 transition-all text-slate-300 cursor-pointer">
                             <QrCode size={18} className="text-purple-400 opacity-80" />
                             <span className="text-[10px] font-bold">Pay (QR)</span>
                           </button>
                           <button onClick={(e) => { e.stopPropagation(); setModalView({ type: 'ACCOUNT', bill }); }} className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl border border-white/5 bg-slate-950/30 hover:bg-slate-900/40 hover:border-white/10 transition-all text-slate-300 cursor-pointer">
                             <UserIcon size={18} className="text-orange-400 opacity-80" />
                             <span className="text-[10px] font-bold">Account</span>
                           </button>
                           <button onClick={(e) => { e.stopPropagation(); setModalView({ type: 'CHAT', bill }); }} className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-xl border border-white/5 bg-slate-950/30 hover:bg-slate-900/40 hover:border-white/10 transition-all text-slate-300 cursor-pointer">
                             <MessageCircle size={18} className="text-green-450 opacity-80" />
             ...
             {modalView.type === 'CHAT' ? (
                ...
                <div className="bg-slate-950/90 border border-white/10 backdrop-blur-3xl rounded-[2rem] w-full max-w-sm overflow-hidden shadow-2xl relative flex flex-col max-h-[80vh]">
                  <div className="p-4 border-b border-white/5 flex justify-between items-center bg-slate-900/40">
                    ...
                    <button onClick={() => setModalView(null)} className="p-1.5 bg-white/5 text-slate-400 hover:text-white rounded-full transition-colors cursor-pointer"><X size={18} /></button>
                  </div>
                    ...
                    {modalView.type === 'QR' && (
                      <div className="flex flex-col items-center py-4">
                        ...
                        <div className="mt-2 text-center text-xs font-black text-cyan-400 bg-cyan-950/30 border border-cyan-500/20 px-4 py-3 rounded-xl tracking-wider">
                          PAYMENT AMOUNT: {formatCurrency(modalView.bill.grandTotal)}
                        </div>
                      </div>
                    )}
                    {modalView.type === 'ACCOUNT' && (
                      <div className="space-y-6">
                        <div className="text-center">
                          ...
                          <div className={`text-4xl font-display font-black ${customer.pendingAmount > 0 ? 'text-red-400' : 'text-green-400'}`}>
                             {formatCurrency(customer.pendingAmount || 0)}
                          </div>
                        </div>
                        <div className="bg-slate-900/40 p-4 rounded-2xl border border-white/5">
                          <div className="text-xs font-bold text-slate-200 mb-3">Customer Details</div>
                          ...
                          <div className="text-sm flex justify-between py-1.5">
                             ...
                             <span className="font-medium text-white">{analytics.TANKER.trips + analytics.CAN.trips + analytics.BOTTLE.trips}</span>
                          </div>
                        </div>
                      </div>
                    )}
